...
import type { AssistantPreferences, SessionMeta, SessionRecord } from '@/types/assistant'
...
function loadGlobalPrefs(): AssistantPreferences {
  try {
...
function loadAllSessionMeta(): Record<string, SessionMeta> {
  try {
...
export const useAssistantStore = defineStore('assistant', () => {
  const preferences = ref<AssistantPreferences>(loadGlobalPrefs())
  const sessions = ref<SessionRecord[]>([])
  const currentSessionId = ref('')
  const sessionMetaMap = ref<Record<string, SessionMeta>>(loadAllSessionMeta())
  ...
  watch(
    ...
    (val) => {
      ...
      if (currentSessionId.value) {
        saveSessionMeta(currentSessionId.value, val)
      }
    },
    { deep: true },
  )

  function saveSessionMeta(sessionId: string, prefs: AssistantPreferences, preview?: string) {
  ...
  function applySessionMeta(sessionId: string) {
  ...
  function setSessions(list: SessionRecord[]) {
    sessions.value = list.map((s) => ({
      ...s,
      meta: getSessionMeta(s.sessionId),
    }))
  }
  ...
  function wrapUserContent(text: string, prefs = preferences.value) {
